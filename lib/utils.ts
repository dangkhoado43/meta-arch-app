const DEFAULT_PREFIX = "metaarch";
const RANDOM_LENGTH = 6;

export const HOSTING_CONFIG_KEY = `${DEFAULT_PREFIX}_hosting_config`;
...
export const toBase36Timestamp = () => Date.now().toString(36);

export const createHostingSlug = (prefix = DEFAULT_PREFIX) => {
    const safePrefix = prefix
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "");

    const timestamp = toBase36Timestamp();
